...
public class CodeRoomController {
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<?> deleteRoom(@PathVariable Long id, @RequestParam Long userId) {
        try {
            boolean deleted = codeRoomService.deleteRoom(id, userId);
            if (deleted) {
                return ResponseEntity.ok().build();
            } else {
                return ResponseEntity.status(HttpStatus.FORBIDDEN)
                    .body(Map.of("error", "只有房间创建者可以删除房间"));
            }
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(Map.of("error", "删除房间失败"));
        }
    }

    @PostMapping("/{id}/run")
    public ResponseEntity<?> runCode(@PathVariable Long id, @RequestBody Map<String, String> request) {
        try {
            ...
            if (code == null || language == null) {
                return ResponseEntity.badRequest()
                    .body(Map.of("error", "代码和语言不能为空"));
            }

            // 验证房间是否存在
            codeRoomService.getRoom(id);
            
            // 运行代码
            String result = codeRoomService.runCode(code, language);
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(Map.of("error", "代码运行失败: " + e.getMessage()));
        }
    }
} 